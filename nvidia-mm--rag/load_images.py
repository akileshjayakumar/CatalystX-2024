    ...
    for i in range(len(f)):
        ...
        for text_block_ctr, (heading_block, content) in enumerate(grouped_text_blocks, 1):
            ...
            if not any(heading_bbox.intersects(table_bbox) for table_bbox in table_bboxes):
                bbox = {"x1": heading_block[0], "y1": heading_block[1],
                        "x2": heading_block[2], "x3": heading_block[3]}
                text_doc = Document(
    ...
    try:
        ...
        for tab in tables:
            if not tab.header.external:
                ...
                tablerefdir = os.path.join(
                    os.getcwd(), "vectorstore/table_references")
                ...
                doc = Document(text=f"This is a table with the caption: {
...
def parse_all_images(filename, page, pagenum, text_blocks):
    ...
    for image_info in image_info_list:
        ...
        image_docs.append(Document(
            text="This is an image with the caption: " + caption, metadata=image_metadata))
    return image_docs
...
def process_ppt_file(ppt_path):
    ...
    for (image_path, page_num), (slide_text, notes) in zip(images_data, slide_texts):
        ...
        processed_data.append(Document(text="This is a slide with the text: " +
                              slide_text + image_description, metadata=image_metadata))
...
def convert_ppt_to_pdf(ppt_path):
    ...
    command = ['libreoffice', '--headless', '--convert-to',
               'pdf', '--outdir', new_dir_path, ppt_path]
...
def load_multimodal_data(files):
    ...
    for file in files:
        ...
        if file_extension in ('.png', '.jpg', '.jpeg'):
            ...
            doc = Document(text=image_text, metadata={
                           "source": file.name, "type": "image"})
